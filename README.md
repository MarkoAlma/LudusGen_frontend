...
| `LudusGen_backend` | Az API szerver. Kezeli a regisztrációt, bejelentkezést, 2FA-t, krediteket, AI provider hívásokat, marketplace műveleteket, fájlfeltöltést és 3D generálási feladatokat. | Node.js, Express, Firebase Admin, Firestore, Cloudinary, Backblaze B2/S3, OpenAI/Anthropic/OpenRouter, Meshy, Tripo, Trellis. |
...
| --- | --- | --- | --- |
| Home | `/` | Nem | Kezdőlap, termékbemutató, fő navigáció és belépési pont. |
...
| AI 3D | `/chat?tab=3d` | Igen | Meshy, Tripo és Trellis alapú 3D generálás, task státusz, modell letöltés. |
...
A kezdőlap a LudusGen belépési felülete. A célja, hogy azonnal láthatóvá tegye: a rendszer AI-alapú játékfejlesztői és kreatív asset platform. Desktopon széles hero felület, fő navigáció, vizuális bemutató blokkok és elsődleges call-to-action gombok jelennek meg.

Funkciói:

- navigáció a fő modulok felé;
- bejelentkezési/regisztrációs modal megnyitása;
- platform értékajánlatának bemutatása;
- AI Studio és marketplace irányába vezető belépési pont.
...
- bal oldali studio sidebar a modulok között;
- modellválasztó sáv a kiválasztott AI típushoz;
...
Az AI 3D a LudusGen egyik legerősebb része: több 3D generálási provider egy felületen jelenik meg.

Funkciók:

- Meshy text-to-3D és image-to-3D task indítása;
- Meshy refine folyamat;
- Tripo text/image/task alapú modellgenerálás;
- Tripo task státusz lekérése és SSE stream;
- Tripo modell proxyzott letöltése;
- Trellis 3D generálás és history kezelés;
- 3D modell fájlok objektumtáras kezelése;
- taskok megszakítása, státuszfigyelése és előzményből visszatöltése.

AI 3D provider összehasonlítás:

| Provider | Mire jó leginkább? | Bemenet | Kimenet | LudusGen szerep |
| --- | --- | --- | --- | --- |
| Meshy | Gyors text-to-3D és image-to-3D asset prototípusok. | Prompt vagy kép. | 3D preview/refined modell, task státusz. | Gyors asset ötletelés és finomítás. |
| Tripo | Összetettebb 3D pipeline-ok, text/image/model alapú taskok, import és letöltés. | Prompt, kép, modellfájl vagy pipeline paraméter. | Task, modell URL, proxyzott letöltés, history. | A legteljesebb 3D workflow: task indítás, státusz, becslés, proxy, library. |
| Trellis | NVIDIA alapú 3D generálás és modellfájl kezelés. | Prompt vagy generálási paraméterek. | 3D modellfájl és history. | Alternatív 3D generálási útvonal és modell kiszolgálás. |
...
| Külső AI szolgáltatók | Generálási feladatok végrehajtása. | OpenRouter/Anthropic/OpenAI válaszok, Meshy/Tripo/Trellis taskok. |
