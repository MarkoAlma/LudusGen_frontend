...
export default function AudioPanel({ selectedModel, userId, getIdToken }) {
  ...
  const [presets, setPresets] = useState(
    DEFAULT_PRESETS.audio.filter((p) =>
      isTTS
        ? ["audio_natural", "audio_dramatic"].includes(p.id)
        : ["music_cinematic", "music_lofi"].includes(p.id),
    ),
  );
  const [activePresetId, setActivePresetId] = useState(
    isTTS ? "audio_natural" : "music_cinematic",
  );
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      const endpoint = isTTS
        ? `${API_BASE}/api/generate-tts`
        : `${API_BASE}/api/generate-music`;
      const body = isTTS
        ? {
      ...
      if (userId) {
        await addDoc(
          collection(db, "audio_generations", userId, selectedModel.id),
          {
            audioUrl: url,
      ...
      {activeTab === "generate" && (
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4 scrollbar-thin">
            ...
            <>
                    ...
                    >
                      <div className="flex items-center gap-1.5 mb-0.5">
                        <Volume2
                          ...
                          style={{
                            color:
                              selectedVoice === voice.id ? color : "#6b7280",
                          }}
                        />
              ...
              >
                ...
                <p className="text-gray-600 text-xs">
                  0.25 = lassú · 1.0 = normál · 4.0 = gyors
                </p>
              </div>
                  ...
                  {TTS_FORMATS.map((fmt) => (
                    <button
                      ...
                      style={{
                        ...
                        transform:
                          audioFormat === fmt ? "scale(1.03)" : "scale(1)",
                      }}
                    >
                      {fmt}
                    </button>
                ...
                <div className="flex flex-wrap gap-1.5">
                  {MUSIC_GENRES.map((g) => (
                    <button
                      ...
                      style={{
                        background:
                          genre === g ? `${color}25` : "rgba(255,255,255,0.04)",
                        border:
                          genre === g
                            ? `1px solid ${color}55`
                            : "1px solid rgba(255,255,255,0.08)",
                ...
                <div className="flex flex-wrap gap-1.5">
                  {MUSIC_MOODS.map((m) => (
                    <button
                      ...
                      style={{
                        background:
                          mood === m ? `${color}25` : "rgba(255,255,255,0.04)",
                        border:
                          mood === m
                            ? `1px solid ${color}55`
                            : "1px solid rgba(255,255,255,0.08)",
                ...
                >
                  ...
                  >
                    <div
                      ...
                      style={{
                        left: instrumental ? "calc(100% - 20px)" : "4px",
                      }}
                    />
                  </button>
                </div>
          ...
          <button
            ...
            style={{
              background:
                (isTTS ? text : musicPrompt).trim() && !isGenerating
                  ? `linear-gradient(135deg, ${color}, ${color}88)`
                  : "rgba(255,255,255,0.05)",
              opacity:
                (isTTS ? text : musicPrompt).trim() && !isGenerating ? 1 : 0.4,
              boxShadow:
                (isTTS ? text : musicPrompt).trim() && !isGenerating
                  ? `0 0 30px ${color}30`
                  : "none",
          ...
          {!presetSaveOpen ? (
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={savePreset}
                className="cursor-pointer px-4 py-2 rounded-xl text-white text-sm font-semibold transition-all hover:opacity-90 active:scale-95"
                style={{
              ...
              <button
                ...
                className="cursor-pointer p-2 rounded-xl text-gray-500 hover:text-gray-300 hover:bg-white/10 transition-all"
