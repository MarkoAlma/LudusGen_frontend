...
import { Check, Sparkles, Zap, Crown, Star, TrendingUp, ArrowRight, Gift, Shield } from 'lucide-react';
  ...
  return (
    <section id="pricing" className="relative py-32 px-4 overflow-hidden">
      ...
      <div className="absolute inset-0 z-0">
        ...
        <div className="absolute bottom-0 right-1/4 w-[600px] h-[600px] bg-cyan-600/20 rounded-full blur-[150px] animate-pulse-slow" style={{ animationDelay: '3s' }} />
        <div className="absolute top-1/2 left-1/2 w-[500px] h-[500px] bg-pink-600/20 rounded-full blur-[150px] animate-pulse-slow" style={{ animationDelay: '6s' }} />
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        ...
        <div className="text-center mb-20">
          ...
          <p className="text-xl md:text-2xl text-gray-300 max-w-3xl mx-auto mb-12 leading-relaxed">
            Rugalmas árazás minden igényre, <span className="font-bold text-white">startup-tól enterprise-ig</span>
          </p>
    ...
    return (
      <div
        ...
        className={`relative group rounded-3xl backdrop-blur-2xl transition-all duration-500 transform
          ${pkg.highlight ? 'lg:-mt-8 lg:mb-8 scale-105' : 'scale-100 hover:scale-105'}
          h-full min-h-[480px]`}
      >
        <div 
        ...
        >
            ...
            <div className="absolute -top-6 left-1/2 transform -translate-x-1/2 z-10">
              <div className="relative">
                ...
                <div className="relative px-6 py-2 bg-gradient-to-r from-purple-600 via-pink-600 to-cyan-600 rounded-full text-sm font-black shadow-2xl flex items-center space-x-2">
                  ...
                  <Star className="w-4 h-4 text-yellow-300 animate-spin-slow" style={{ animationDirection: 'reverse' }} />
                </div>
              </div>
            </div>
          )}
          ...
          <div className="mb-8 relative">
            <div className={`inline-flex p-5 rounded-2xl bg-gradient-to-br ${pkg.gradient} shadow-2xl transform transition-all duration-500 ${
              isHovered ? 'scale-110 rotate-6' : 'scale-100 rotate-0'
            }`}>
              <Icon className="w-10 h-10 text-white" />
              {isHovered && (
                <div className="absolute inset-0 animate-ping">
                  <div className={`w-full h-full rounded-2xl bg-gradient-to-br ${pkg.gradient} opacity-50`} />
                </div>
              )}
            </div>
            {isHovered && (
          ...
          <h3 className={`text-3xl font-black mb-6 transition-all duration-300 ${
            isHovered ? 'text-transparent bg-gradient-to-r from-purple-300 via-pink-300 to-cyan-300 bg-clip-text' : 'text-white'
          }`}>
            {pkg.name}
          </h3>
          ...
          <ul className="space-y-4 mb-10">
            {pkg.features.map((feature, i) => (
              ...
              >
                <div className={`relative p-1 rounded-full bg-gradient-to-br ${pkg.gradient} mr-3 flex-shrink-0 mt-0.5 transition-all duration-300 group-hover/item:scale-110 group-hover/item:rotate-12`}>
                  <Check className="w-4 h-4 text-white" />
                  <div className={`absolute inset-0 rounded-full bg-gradient-to-br ${pkg.gradient} blur-md opacity-0 group-hover/item:opacity-100 transition-opacity animate-pulse`} />
                </div>
          ...
          <button className={`relative w-full cursor-pointer py-4 rounded-2xl font-black text-lg transition-all duration-500 overflow-hidden group/btn ${
            ...
            {pkg.highlight ? (
              <>
                <div className={`absolute inset-0 bg-gradient-to-r ${pkg.gradient} animate-gradient-x`} />
                ...
                <div className={`absolute inset-[-2px] rounded-2xl bg-gradient-to-r ${pkg.gradient} opacity-0 group-hover/btn:opacity-100 blur-lg transition-opacity`} />
              </>
            ) : (
              <>
                <div className="absolute inset-0 bg-white/10 group-hover/btn:bg-white/20 transition-colors" />
                <div className={`absolute inset-0 bg-gradient-to-r ${pkg.gradient} opacity-0 group-hover/btn:opacity-20 transition-opacity`} />
              </>
            )}
            <span className="relative z-10 flex items-center justify-center space-x-2 uppercase tracking-wider">
              <span>Kezdés</span>
              <ArrowRight className="w-5 h-5 group-hover/btn:translate-x-2 transition-transform" />
              {pkg.highlight && <Zap className="w-5 h-5 animate-pulse" />}
            </span>
            {pkg.highlight && (
        ...
        <div className="flex flex-wrap justify-center gap-8 mb-12">
          {[
            { icon: Shield, text: 'SSL Titkosítás', color: 'from-green-400 to-emerald-400' },
            { icon: Zap, text: '99.9% Uptime', color: 'from-yellow-400 to-orange-400' },
            { icon: Crown, text: 'Prémium Support', color: 'from-purple-400 to-pink-400' },
            { icon: Star, text: '10,000+ Elégedett Ügyfél', color: 'from-cyan-400 to-blue-400' }
          ].map((badge, idx) => {
            ...
            return (
              ...
              >
                <BadgeIcon className={`w-5 h-5 text-${badge.color.split('-')[1]}-400`} />
        ...
        <div className="relative group cursor-pointer">
          ...
          <div className="relative text-center p-8 bg-gradient-to-r from-white/5 via-white/10 to-white/5 backdrop-blur-xl border border-white/10 rounded-2xl">
            <div className="flex items-center justify-center space-x-3 mb-4">
              ...
              <Shield className="w-8 h-8 text-green-400 animate-pulse" style={{ animationDelay: '1s' }} />
            </div>
            <p className="text-gray-400 max-w-2xl mx-auto">
              Nem vagy elégedett? <span className="text-white font-bold">Visszafizetjük a teljes összeget</span> - kérdések nélkül!
            </p>
          </div>
        </div>
      ...
      <style jsx>{`
        @keyframes float-random {
          ...
          50% { transform: translate(-15px, -40px); opacity: 0.5; }
