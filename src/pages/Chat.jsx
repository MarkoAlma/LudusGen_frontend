...
import { Send, Sparkles, Bot, User, Zap, Brain, Cpu, Crown, Star, CircleDot, Wand2, MessageSquare } from 'lucide-react';

export default function AIChat() {
  const [selectedAI, setSelectedAI] = useState('gpt4');
  const [messages, setMessages] = useState([
    { role: 'assistant', content: 'Szia! Válassz egy AI modellt és kezdjük el a beszélgetést! 🚀', model: 'system' }
  ]);
  ...
  return (
    <div className="min-h-screen flex items-center justify-center p-4 relative overflow-hidden" style={{
      ...
      <div className="relative w-full h-[90vh] flex gap-4 mt-auto z-10">
        ...
        <div className="w-80 h-full rounded-3xl backdrop-blur-2xl border border-white/10 p-6 overflow-y-auto"
          style={{
          ...
          <div className="space-y-3">
            {aiModels.map((model) => (
              <button
                ...
                style={{
                    cursor:'pointer',
                  background: selectedAI === model.id 
                    ? `linear-gradient(135deg, ${model.color}30, ${model.color}10)` 
                    : 'rgba(255, 255, 255, 0.03)',
                  border: selectedAI === model.id 
              ...
              >
                ...
                <div className="relative flex items-start gap-3">
                  <div className={`p-2.5 rounded-xl bg-gradient-to-br ${model.gradient} text-white flex-shrink-0 shadow-lg`}>
                    {model.icon}
                  </div>
                  ...
                  <div className="flex-1 text-left">
                    ...
                    <span className={`inline-block px-2 py-0.5 rounded-full text-xs font-semibold bg-gradient-to-r ${model.gradient} text-white`}>
                      {model.badge}
                    </span>
                  </div>
                </div>
          ...
          <div className="mt-6 p-4 rounded-2xl" style={{
            ...
            <div className="space-y-1 text-xs text-gray-400">
              ...
              <div className="flex justify-between">
                ...
                <span className="text-white font-semibold">{getModelInfo(selectedAI).name}</span>
              </div>
            </div>
          </div>
        </div>
        ...
        {/* Chat Area */}
        <div className="flex-1 h-full rounded-3xl backdrop-blur-2xl border border-white/10 flex flex-col overflow-hidden"
          style={{
            background: 'rgba(15, 15, 35, 0.6)',
            boxShadow: '0 8px 32px rgba(0, 0, 0, 0.3), inset 0 1px 0 rgba(255, 255, 255, 0.1)'
          }}>
          ...
          {/* Chat Header */}
          <div className="px-6 py-4 border-b border-white/10 backdrop-blur-xl flex items-center justify-between"
            style={{ background: 'rgba(255, 255, 255, 0.02)' }}>
            <div className="flex items-center gap-3">
              <div className={`p-3 rounded-xl bg-gradient-to-br ${getModelInfo(selectedAI).gradient} shadow-lg`}>
                {getModelInfo(selectedAI).icon}
              </div>
              <div>
                <h3 className="font-bold text-white text-lg">{getModelInfo(selectedAI).name}</h3>
            ...
            <div className="flex items-center gap-2 px-4 py-2 rounded-full" style={{
              ...
              <span className="text-sm text-gray-300 font-semibold">{messages.length} üzenet</span>
            </div>
          </div>
          ...
          <div className="flex-1 overflow-y-auto p-6 space-y-4 scrollbar-thin scrollbar-thumb-white/10 scrollbar-track-transparent">
              ...
              return (
                ...
                >
                  ...
                  <div className={`w-10 h-10 rounded-xl flex items-center justify-center flex-shrink-0 shadow-lg ${
                    isUser ? 'bg-gradient-to-br from-cyan-500 to-blue-500' : `bg-gradient-to-br ${modelInfo.gradient}`
                  }`}>
                    {isUser ? <User className="w-5 h-5 text-white" /> : modelInfo.icon}
                  </div>

                  {/* Message Bubble */}
                  <div className={`max-w-xl ${isUser ? 'items-end' : 'items-start'} flex flex-col gap-1`}>
                    <div className={`px-5 py-3 rounded-2xl backdrop-blur-sm ${
                      isUser 
            ...
            {isTyping && (
              <div className="flex gap-3 animate-slide-up">
                <div className={`w-10 h-10 rounded-xl bg-gradient-to-br ${getModelInfo(selectedAI).gradient} flex items-center justify-center shadow-lg`}>
                  {getModelInfo(selectedAI).icon}
                </div>
          ...
          {/* Input Area */}
          <div className="p-4 border-t border-white/10 backdrop-blur-xl" style={{ background: 'rgba(255, 255, 255, 0.02)' }}>
            <div className="flex gap-3 items-end">
              ...
              <button
                ...
                className={`p-4 rounded-2xl transition-all duration-300 shadow-lg ${
                  input.trim() 
                    ? `bg-gradient-to-br ${getModelInfo(selectedAI).gradient} hover:scale-105 hover:shadow-2xl` 
