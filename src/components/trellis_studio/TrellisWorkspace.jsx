          ...
          {VIEW_MODES.map(v => (
            <Tooltip key={v.id} text={v.tip} side="bottom">
              <button
                ...
                className={`px-4 py-1.5 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all duration-300 ${viewMode === v.id ? 'bg-primary/20 text-white shadow-[0_0_20px_rgba(139,92,246,0.3)] border border-primary/30' : 'text-zinc-600 hover:text-zinc-400'
                  }`}
              >
                {v.label}
              </button>
